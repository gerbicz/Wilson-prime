...
// 0    50M  2M       103MB RAM ?      569 sec.
// 0    10B  10M      841MB RAM ??     approx. 420000 sec.; with older code
...
void modproducttree(mpz_ptr,mpz_t,lli,lli);
...
void mpzred2(mpz_ptr,mpz_t,mpz_t,mpz_t);
...
void modproducttree (mpz_ptr result,mpz_t m,lli pos1,lli pos2)  {
// Mulptiplication by binary splitting with product tree.
// result=prod(i=pos1,pos2,i) or prod(i=pos1,pos2,i)%m
  if(pos1>pos2)  {
     mpz_set_ui(result,1);// empty product is one.
     return;
  }
  ...
  if(pos2-pos1+1>2*interval)  {
     mpz_t temp2;
     mpz_init(temp);
     mpz_init(temp2);
     modproducttree(temp2,m,pos1,mid);
     modproducttree(temp,m,mid+1,pos2);
     mpzred2(result,temp,temp2,m);
     return;
  }
  modproducttree(result,m,pos1,mid);
  mpz_init(temp);
...
// 0<=offset<4, so four different strategy for func
    ...
    if(offset&1)  {
        if(N-n1>2*interval)  {
           ...
           modproducttree(T,temp,n1,N-1);
           mpz_clear(temp);del=0;
        }
        else  producttree2(T,n1,N-1);

        if(offset>>1)  remaindertree2(T,vec,res,pos1,pos2);
...
// Compute (r1*m+r0)^2 mod m^2
    ...
    mpz_mul(s1,s1,r0);
    mpz_mul_2exp(s1,s1,1);
    mpz_clear(r1);
    mpz_fdiv_r(temp,s1,m);
    mpz_clear(s1);

    mpz_mul(s0,s0,s0);
    mpz_fdiv_qr(s0,r0,s0,m);

    mpz_add(temp,temp,s0);
    mpz_clear(s0);
    if(mpz_cmp(temp,m)>=0)mpz_sub(temp,temp,m);
    mpz_init_set(r1,temp);
    mpz_clear(temp);
...
void mpzred2(mpz_ptr result,mpz_t n1,mpz_t n2,mpz_t m)  {
// compute n1*n2 % m^2
    mpz_t r0,r1,s0,s1,temp;
    mpz_init(r1);
    mpz_init(r0);
    mpz_init(s1);
    mpz_init(s0);
    ...
    mpz_fdiv_qr(r1,r0,n1,m);
    mpz_clear(n1);

    mpz_fdiv_qr(s1,s0,n2,m);
    mpz_clear(n2);
   ...
   if(in!=NULL)  {
    ...
    while(mpz_cmp(n,en)<=0)  {
          ...
          if((ct==BR||mpz_cmp(n2,en)>0)&&numprimes>0)  {
                ...
                if(1)func((firstprime[0]-1)/dd+1,(mpztolonglong(prec)-1)/dd+1,A,res,firstprime,dd,0,ct-1,1);
                ...
                for(i=0;i<ct;i++)  {
                    ...
                    func((pr[0]-1)/dd+1,(pr[numprimes-1]-1)/dd+1,g,res2,pr,dd,0,numprimes-1,3);
